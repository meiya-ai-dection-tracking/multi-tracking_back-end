...
# （1）预测文本(.txt)放置在./input/detection-results中，一个文本代表一张图片，一张图片内可有多个预测对象
#   每行格式为<class_name> <confidence> <left> <top> <right> <bottom>
# （2）真值文本(.txt)放置在./input/ground-truth中，一个文本代表一张图片，一张图片内可有多个真值对象
#   每行格式为<class_name> <left> <top> <right> <bottom> [<difficult>]
#   （The difficult parameter is optional, use it if you want the calculation to ignore a specific detection.）
# （3） 原始图片(.jpg)放置在./input/images-optional中，尺寸要求暂无
#   同一图片以上三个文件夹对应的文件名相同。
# ------------------------------------------------------------
input_files_path = 'input'
output_files_path = "output"
MINOVERLAP = 0.5  # default value (defined in the PASCAL VOC2012 challenge) 阈值

parser = argparse.ArgumentParser()  # argparse库可以实现直接在命令行中就可以向程序中传入参数并让程序运行
...
GT_PATH = os.path.join(os.getcwd(), input_files_path, 'ground-truth')
DR_PATH = os.path.join(os.getcwd(), input_files_path, 'detection-results')
# if there are no images then no animation can be shown
IMG_PATH = os.path.join(os.getcwd(), input_files_path, 'images-optional')
